...
function getProducts(request, response) {
  ...
  if (category_id > 0) {
    ...
    const sqlOpdracht = db.prepare('SELECT products.id AS id, products.code AS code, products.price AS price, products.description AS description, products.name AS name, type.id AS type_id2, type.name AS type_name, merk.id AS merk_id2, merk.name AS merk_name, land.id AS land_id2, land.name AS land_name FROM products JOIN type ON type.id = products.type_id JOIN merk ON merk.id = products.merk_id JOIN land on land.id = products.land_id ORDER BY id ASC; ')
