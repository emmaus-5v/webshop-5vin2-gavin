...
function getProducts(request, response) {
  ...
  if (category_id > 0) {
    ...
    const sqlOpdracht = db.prepare('SELECT products.id AS id, products.code AS code, products.price AS price, products.description AS description, products.name AS name, type.id AS type_id2, type.name AS type_name, merk.id AS merk_id2, merk.name AS merk_name, land.id AS land_id2, land.name AS land_name, motor.id AS motor_id2, motor.name AS motor_name, waardering.id AS waardering_id2, waardering.name AS waardering_name FROM products JOIN type ON type.id = products.type_id JOIN merk ON merk.id = products.merk_id JOIN land ON land.id = products.land_id JOIN motor ON motor.id = products.motor_id JOIN waardering ON waardering.id = products.waardering_id ORDER BY id ASC; ')
